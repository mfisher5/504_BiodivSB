...
lmdat <- mydat %>%
  dplyr::select(-purpuratus_abundance,-franciscanus_abundance, -max_po4, -mean_po4,
                -mean_pon,-max_pon,-summer_mean_temp,-winter_mean_temp,-winter_var_temp, -summer_var_temp)
...
fit.a <- lm(mapy_mean_size ~ urchin_abundance+site+
              mean_no2_no3+mean_ammonia+mean_tchl+
...
fit.abund.a <- lm(mapy_mean_abund ~ urchin_abundance+site+
              mean_no2_no3+mean_ammonia+mean_tchl+
              prev_mean_abund+site*prev_mean_abund,data=lmdat)
scp <- list(lower = ~ 1,
           upper = ~ urchin_abundance+site+
              mean_no2_no3+mean_ammonia+mean_tchl+
              prev_mean_abund+site*prev_mean_abund)
...
A bunch of things look weird here: the TA plot shows a fan shape, suggesting that our estimates are less precise at higher values. The model also violates the assumption of normality, with a right tail on the residuals. 
```{r}
par(mfrow=c(2,2))
plot(fit.abund.f)
```
