...
```{r}
kelp <- read.csv(here::here("data","Annual_Kelp_All_Years_20181127.csv")) %>%
  clean_names() %>%
  remove_empty("rows")
...
```{r}
fish <- read.csv(here::here("data","Annual_fish_comb_20181127.csv")) %>%
  clean_names() %>%
  remove_empty("rows")
...
```{r}
invert <- read.csv(here::here("data","Annual_Quad_Swath_All_Years_20181127.csv")) %>%
  clean_names() %>%
  remove_empty("rows")
...
I'm going to grab the data for the mainland sites, for 2001 onward, and focus only on abundance of kelp (not size). Abundance is measured as number of fronds
```{r}
kelp_thin <- dplyr::select(kelp, year, month, site,transect,quad,side,sp_code,scientific_name,group,fronds) %>%
  filter(year >= 2001)
```
<br>
