...
```{r}
n <- dim(size_dat)[1]
fit.a <- lm(log(mapy_mean_hld) ~ site + urchin_presence + urchin_abundance + 
...
```{r}
fit.e <- lm(log(mapy_mean_hld) ~ 1, data=size_dat)
...
Forward and backward selection produces different models. Both of them are pretty terrible, but the R-squared is slightly better for the larger model
```{r}
summary(fit.ff); summary(fit.fb)
```
<br>
...
Forward and backward selection produces very different models, and also very different models from BIC-based selection.
```{r}
summary(fit.fb); summary(fit.ff)
```
<br>

The r-squared for the largest model is better than the other three we've looked at, although still below 0.50. There appear to be some issues with normality of residuals.
```{r}
plot(fit.ff)
hist(fit.ff$residuals)
```
<br>
...
```{r}
glmfit.a <- glm(mapy_mean_hld ~ site + urchin_presence + urchin_abundance +
              mean_ammonia + mean_no2_no3 + mean_po4 + mean_tchl + mean_pon +
              summer_mean_temp + summer_var_temp +
              prev_mean_hld + site:prev_mean_hld, data=size_dat, family=Gamma(link='log'))
...
```{r}
glmfit.e <- glm(mapy_mean_hld ~ 1, data=size_dat, family=Gamma(link='log'))
glmfit.ff <- step(glmfit.e, scope=scp, direction = "forward",  k = log(n))
```
<br>

Forward and backward selection chose the same model, with just site as a predictor. 
```{r}
summary(glmfit.f)
anova(glmfit.e,glmfit.f, test="F")
plot(glmfit.f); plot(glmfit.f, which=4)
...
Forward and backward selection chose different models, but match the AIC-based models from the OLS regression.
<br>
