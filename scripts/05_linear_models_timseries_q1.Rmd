...
kelp_prev <- read.csv(here::here("data","kelp_prediction_data.csv")) %>%
  clean_names() %>%
  dplyr::select(year,month,site,transect,
...
Nitrogen and phosphorus are correlated, as are particulate organic carbon and nitrogen. Particulate organic nitrogen is highly correlated with total chlorophyll. So we'll just concentrate on nitrogetn (`no2_no3`) and particulate organic carbon (`poc`).


```{r}
lmdat <- mydat %>%
  dplyr::select(-purpuratus_abundance,-franciscanus_abundance,-summer_var_temp,-winter_var_temp)
corrplot(cor(lmdat[,c(9,14:18,20)],use="complete.obs"),type="upper")
```
<br>
...
fit.a <- lm(mapy_mean_size ~ urchin_abundance+site+
              mean_no2_no3+mean_ammonia+mean_tchl+mean_pon+mean_poc+
              summer_mean_temp+winter_mean_temp+
              prev_mean_size+site*prev_mean_size,data=lmdat)
scp <- list(lower = ~ 1,
           upper = ~ urchin_abundance+site+
              mean_no2_no3+mean_ammonia+mean_tchl+mean_pon+mean_poc+
             summer_mean_temp+winter_mean_temp+
              prev_mean_size+site*prev_mean_size)
...
It looks like this model may violate the normality assumption, but there don't appear to be any outliers. 
```{r}
check_model(fit.f)
```
<br>
...
The first issue with this model is that the nitrogen concentration and the winter temperature are moderately correlated. A few other things look weird here: the TA plot shows a fan shape, suggesting that our estimates are less precise at higher values. The model also violates the assumption of normality, with a right tail on the residuals. 
```{r}
check_model(fit.abund.f)
```
