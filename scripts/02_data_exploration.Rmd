...
waterchem <- read.csv(here::here("data","Annual_CTD_WaterChemistry_MeanMax_2000-2018.csv")) %>%
  filter(target_depth_m==1) %>% dplyr::select(-target_depth_m)
...
```{r}
kelp_chem_means <- filter(dat2_water, site %in% unique(waterchem$station)) %>% dplyr::select(5:8,14:18)
kelp_chem_max <- filter(dat2_water, site %in% unique(waterchem$station)) %>% dplyr::select(5:13)
...
corrplot(cor(kelp_temp[,c(5:8,19:22)],use="complete.obs"), type="upper")
```
