...
```{r}
kelp <- read.csv(here::here("data","Annual_Kelp_MeanSize_MeanAbund_2001-2018.csv")) %>% dplyr::select(-sp_code,-scientific_name, -group) %>%
  rename(kelp_mean_abund=mean_abund, kelp_max_abund=max_abund,kelp_mean_size=mean_size,kelp_max_size=max_size)
...
```{r}
abund_fish <- q1dat_fish %>% dplyr::select(-kelp_mean_size,-kelp_max_size)
corrplot(cor(abund_fish[,6:16],use="complete.obs"), type="upper")
```
<br>
...
```{r}
size_fish <- q1dat_fish %>% dplyr::select(-kelp_mean_abund,-kelp_max_abund) %>%
  filter(!is.na(kelp_mean_size))
corrplot(cor(size_fish[,6:16],use="complete.obs"), type="upper")
```
<br>

### Invertebrate Community Composisiont
How does invertebrate community abundance and diversity vary according to giant kelp abundance?

```{r}
abund_invert <- q1dat_invert %>% dplyr::select(-kelp_mean_size,-kelp_max_size, -survey)
corrplot(cor(abund_invert[,6:16],use="complete.obs"), type="upper")
```
<br>
...
```{r}
size_invert <- q1dat_invert %>% dplyr::select(-kelp_mean_abund,-kelp_max_abund,-survey) %>%
  filter(!is.na(kelp_mean_size))
corrplot(cor(size_invert[,6:16],use="complete.obs"), type="upper")
```
<br>
...
```{r}
corrplot(cor(dat2_urchins[,6:12],use="complete.obs"), type="upper")
```
<br>

How does kelp size / abundance vary according to water chemistry?
```{r}
kelp_chem_means <- filter(dat2_water, site %in% unique(waterchem$station)) %>% filter(!is.na(target_depth_m)) %>% dplyr::select(-target_depth_m) %>% dplyr::select(6:9,15:19)
kelp_chem_max <- filter(dat2_water, site %in% unique(waterchem$station)) %>% filter(!is.na(target_depth_m)) %>% dplyr::select(-target_depth_m) %>% dplyr::select(6:14)
