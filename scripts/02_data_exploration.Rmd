...
```{r}
kelp_chem_means <- filter(dat2_water, site %in% unique(waterchem$station)) %>% dplyr::select(5:8,15:19)
kelp_chem_max <- filter(dat2_water, site %in% unique(waterchem$station)) %>% dplyr::select(5:14)
...
corrplot(cor(kelp_temp[,c(5:8,21:24)],use="complete.obs"), type="upper")
```
